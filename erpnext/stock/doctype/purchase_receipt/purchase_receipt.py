...
class PurchaseReceipt(BuyingController):
	...
	def on_submit(self):
		for item in self.get("items"):
			frappe.db.sql("UPDATE `tabPurchase Order Item` SET `receipt_qty`='"+str(item.qty)+"' WHERE `parent`='" +self.purchase_series+"' AND `item_code`='" +item.item_code+"'")
...
def make_quality(source_name, target_doc=None):
	doc = get_mapped_doc("Purchase Receipt", source_name, {
		...
		"Purchase Receipt Item": {
			"doctype": "Quality Inspection Item",
			"field_map": {
...
@frappe.whitelist()
def make_pur_invoice(source_name, target_doc=None):
	doclist = get_mapped_doc("Purchase Receipt", source_name,	{
		"Purchase Receipt": {
			"doctype": "New Invoice",
			"field_map": {
		...
		"Purchase Receipt Item": {
			"doctype": "New Purchase Invoice Item",
		},
		"Purchase Taxes and Charges": {
			"doctype": "Taxes and Charges"
		}
