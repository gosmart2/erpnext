...
class StockEntry(StockController):
	...
	def on_submit(self):
		...
		if self.purpose == "Material Issue":
			frappe.db.sql("UPDATE `tabMaterial Request` SET `overall_status`='Issued' WHERE name='" +self.material_series+"'")
			for item in self.get("items"):
				frappe.db.sql("UPDATE `tabMaterial Request Item` SET `item_status`='Issued' WHERE `parent`='" +self.material_series+"' AND `item_code`='" +item.item_code+"'")
	def on_cancel(self):
...
@frappe.whitelist()
def make_stock(source_name, target_doc=None):
